...
if remakedata:
    ...
    # deep indexes of the pieces to be highlighted
    han_highlight = []
    jazz_highlight = []
    ...
    for i in range( len( session_folders ) ):
        ...
        han2show = ['han0269.xml']
        jazz2show = ['benny_and_the_jets.xml']
        tmp_feats = []
    ...
    for j in range( len( han2show ) ):
        tmp_idx = all_names.index( han2show[j] )
        han_highlight.append( tmp_idx )
        tmp_idx = all_names.index( jazz2show[j] )
        jazz_highlight.append( tmp_idx )
    # PCA
